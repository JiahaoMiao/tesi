...
now we substitute \cref{eq:renormalization scale dependence} into $\lambda = b_0 \alpha_s(Q^2) L$ up to $\order{\alpha_s^2(\mu^2)}$ and obtain: 
...
and formally expand in powers of $\alpha_s(\mu^2)$, now $\lambda = \lambda(\mu^2)$:
